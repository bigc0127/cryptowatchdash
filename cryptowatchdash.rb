...
def setup
  $time = 10
  # opening file selected from list above
...
if $ownlrc.to_f > 0
  ...
  # showing the value of each coin owned in proportion to how much of each coin is owned
  if $ownbtc.to_f > 0
...
if $ownlrc.to_f > 0
  ...
  # showing what value 1 coin needs to be in order to match the amount of money spent on each crypto coin
  if $ownbtc.to_f > 0
...
def advanceoutput
  # if function used to allow a color highlight to the total profits/losses listed below
  if $totalearnins >= 0
  print "Total Gains: "
